...
async fn get_users(mut request_ctx: RequestContext<'_>) -> OrcaResult {
...
async fn update_user(
    ...
    _user.name = Set(body
        ...
        .unwrap_or(&_user.name.take().unwrap())
        .to_owned());
    _user.first_name = Set(body
        .get("first_name")
        .and_then(Value::as_str)
        .unwrap_or(&_user.first_name.take().unwrap())
        .to_owned());
    _user.last_name = Set(Some(
        body.get("last_name")
            .and_then(Value::as_str)
            .unwrap()
            .to_owned(),
    ));
    _user.is_active = Set(body
        .get("is_active")
        .and_then(Value::as_bool)
        .unwrap()
        .to_owned());
    let _response = _user
        .save(&request_ctx.database())
        .await
        .map_err(|data| OrcaError::DBError(data))?;
    generate_success_response(None, None, None)
}

async fn delete_user(mut request_ctx: RequestContext<'_>, path: Path<i32>) -> OrcaResult {
    let id = path.into_inner();
    let existing_user = user::Entity::find_by_id(id)
        .one(&request_ctx.database())
        .await
    ...
    existing_user
        .delete(&request_ctx.database())
        .await
        .map_err(|data| OrcaError::DBError(data))?;
    Ok(HttpResponse::from(HttpResponse::NoContent()))
}

async fn get_user(mut request_ctx: RequestContext<'_>, path: Path<i32>) -> OrcaResult {
    let id = path.into_inner();
    let existing_user = user::Entity::find_by_id(id)
        .filter(user::Column::IsActive.eq(true))
        .one(&request_ctx.database())
