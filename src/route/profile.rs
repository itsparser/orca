...
pub fn profile_config(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/profile")
            ...
            .route(
                "/{id}/data/{data_id}",
                web::delete().to(delete_profile_data),
            ),
    );
}

/// list all the User Profile in the Orca Application
async fn get_profiles(mut request_ctx: RequestContext<'_>) -> OrcaResult {
    let _profiles = profile::Entity::find()
...
async fn create_profile(
    profile: web::Json<profile::Profile>,
    mut request_ctx: RequestContext<'_>,
) -> OrcaResult {
...
async fn get_profile(path: Path<i32>, mut request_ctx: RequestContext<'_>) -> OrcaResult {
    let id = path.into_inner();
    let existing_profile: Option<profile::Model> = profile::Entity::find_by_id(id)
        .one(&request_ctx.database())
        .await
        .map_err(|data| OrcaError::DBError(data))?;
    if existing_profile.is_some() {
        let mut profile = existing_profile.unwrap();
        let profile_data = profile_data::Entity::find()
            .filter(profile_data::Column::ProfileId.eq(id))
            .order_by_asc(profile_data::Column::Id)
            .all(&request_ctx.database())
            .await
            .map_err(|data| OrcaError::DBError(data))?;
    ...
    return generate_success_response(
        ...
        Some("Profile Not Found".parse().unwrap()),
        None,
    );
}

/// Delete the Single profile With ID
async fn delete_profile(path: Path<i32>, mut request_ctx: RequestContext<'_>) -> OrcaResult {
    let id = path.into_inner();
    let _profile = profile::Entity::delete_by_id(id)
...
async fn add_profile_data(
    path: Path<i32>,
    _profile_data: web::Json<profile_data::ProfileData>,
    mut request_ctx: RequestContext<'_>,
) -> OrcaResult {
    let id = path.into_inner();
...
async fn get_profile_data(path: Path<i32>) -> impl Responder {
    let id = path.into_inner();
    let mut request_ctx = RequestContext::default();
    let _profiles = profile_data::Entity::find()
        .filter(profile_data::Column::ProfileId.eq(id))
        .order_by_asc(profile_data::Column::Id)
        .all(&request_ctx.database())
        .await;
...
async fn delete_profile_data(path: Path<(i32, i32)>) -> impl Responder {
    ...
    let mut request_ctx = RequestContext::default();
    let _profile_data = profile_data::Entity::delete_by_id(data_id)
        .exec(&request_ctx.database())
        .await;
