...
use sea_orm::{
    ActiveModelTrait, ColumnTrait, Condition, EntityTrait, JsonValue, QueryFilter, QueryOrder, Set,
};
...
async fn signin(
    ...
    .await
    .map_err(|data| OrcaError::DBError(data))?;
    let _usr: serde_json::Value =
        serde_json::from_str(serde_json::to_string(&usr).unwrap().as_str()).unwrap();
...
async fn get_redirection_url() -> impl Responder {
    ...
    HttpResponse::Found()
        .append_header(("location", redirection_url))
        .finish()
}
